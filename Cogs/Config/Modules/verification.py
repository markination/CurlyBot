...
class WelcomeEmbedCreation(discord.ui.View):
    ...
    async def EmbedFinished(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        module = await get_module_configuration(interaction.guild.id, "VerifyModule")  
        db = self.mongo["Curly"]["Config"]

        if module is None:
            await interaction.response.send_message("The VerifyModule configuration was not found.", ephemeral=True)
            return
        ...
        result = await db.update_one(
            {"guild_id": guild_id},  
            {"$set": {"VerifyModule.content": json, "VerifyModule.msg_content": self.msg_content}} 
        )

        if result.modified_count > 0:
            await interaction.response.send_message("The verify embed has been updated successfully!", ephemeral=True)
        else:
            await interaction.response.send_message("No changes were made. Please check if the VerifyModule exists.", ephemeral=True)
...
class VerifiedRole(discord.ui.RoleSelect):
    def __init__(self, mongo_connection, ctx):
        self.ctx = ctx
        self.mongo = mongo_connection

        super().__init__(placeholder="Select a verified role", max_values=1, min_values=1, row=2)
...
class UnverifiedRole(discord.ui.RoleSelect):
    def __init__(self, mongo_connection, ctx):
        ...
        super().__init__(placeholder="Select a unverified role", max_values=1, min_values=0, row=2)
